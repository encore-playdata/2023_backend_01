...
System.out.println("arr : " + arr);  // 단순히 기존 배열의 주소값을 참조하는 arr변수는 original 변수와 주소값이 같은 것을 확인할 수 있다

System.out.println("copy : " + copy); // colne을 통해 복제한 copy 변수는 주소값이 original과 다르게 나오는걸 확인 할 수 있다.
...
int[][] copy2 = original2.clone(); // 이차원 배열을 담을 박스를 null 값으로 먼저 만들어준다.

int[][] arr2 = original2;
int in = 0;
for (int[] c:copy2) { // for문을 통해 clone
	int[] qq = copy2[in];
	qq = c.clone();
System.out.println(c);
System.out.println(c.clone());
System.out.println(copy2[in]);
in++;
}
...
// 단순히 기존 배열의 주소값을 참조하는 arr변수는 original 변수와 주소값이 같은 것을
//확인할 수 있다

System.out.println(copy2); 
// colne을 통해 복제한 copy 변수는 주소값이
//original과 다르게 나오는걸 확인 할 수 있다.
